...
The `config.toml` generated by `init` is the smallest config that keeps `example.toml` valid. Add only the values you want to use from there. Arrays for unused config types may stay empty.
